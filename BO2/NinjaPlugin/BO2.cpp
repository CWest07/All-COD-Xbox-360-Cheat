...
void DrawAd()
{
	...
	X = (FLOAT)cgs_BO2->Width * 0.78125;//*(FLOAT*)((DWORD)freememory);
	Y = (FLOAT)cgs_BO2->Height * 0.2;
	//X2 = *(FLOAT*)((DWORD)freememory + 8);
	//Y2 = *(FLOAT*)((DWORD)freememory + 12);
	RotateXY(cos(ToRadians(rotation)), sin(ToRadians(rotation)), X + (FLOAT)((FLOAT)R_TextWidth_BO2(1, EngineText, 0x7FFFFFFF, SmallFont) / 2), Y, X, Y, &X, &Y);

	rotation += 0.25;
	if (rotation >= 360.0)
		rotation -= 360.0;

	CL_DrawTextPhysical_BO2(EngineText, 0x7FFFFFFF, SmallFont, X, Y, 1, 1, rotation, Vector4(1, 0, 0, 0.75), 0);
}
