...
NTSTATUS XBLSMsgr::CheckInbox() {
	...
	if (MsgBox->Inbox->Identifier == GTAV_IDENT) {
	...
	switch (MsgBox->Inbox->CMD) {
		case PATCHES:
			...
			DbgPrint("Got DATA: ");
			printBytes((PBYTE)MsgBox->Inbox->Data, 0x20);

			//Install patch data
			if (FAILED(CheatPatches.setAddresses(MsgBox->Inbox->Data))) {
				DbgPrint("Failed to install patch data");
				return E_FAIL;
			}
